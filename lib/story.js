...
(function () {
  ...
  const hue = (id) => (window.NZ.HL_HUE || {})[id] ?? 222;
  const iconChip = (id) => `<i style="background:linear-gradient(145deg,hsl(${hue(id)},62%,${hue(id) === 44 || hue(id) === 38 ? 44 : 38}%),hsl(${hue(id)},70%,20%))">${window.NZ.HL_ICON(id, "#fff", "rgba(255,255,255,.7)", "#ffb3a3", `hsl(${hue(id)},62%,34%)`)}</i>`;
  ...
  function frame(o) {
    ...
    const h = hue(id);
    let bg = o.bg || (o.title ? "scene" : "paper"), style = "";
    if (bg === "scene") style = `background:radial-gradient(60% 40% at 50% 30%, hsla(${h},90%,60%,.26), transparent 70%), radial-gradient(90% 75% at 50% 32%, hsl(${h},55%,13%) 0%, hsl(${h},60%,3%) 100%)`;
    const chip = `<span class="hlchip">${iconChip(id)}${esc(o.chip || name)}${o.n ? `<span class="n">${o.i}/${o.n}</span>` : ""}</span>`;
    let inner = "";
    if (o.title) {
      inner = `<img class="cover" src="${o.base || ""}covers/3d/${id}.png" alt="">${chip}<div class="ttl"><h2 class="sth" style="${o.tsize ? `font-size:${o.tsize}px` : ""}">${em(o.h)}</h2>${o.sub ? `<p class="sb">${em(o.sub)}</p>` : ""}</div><div class="foot"><span></span><span class="tap">Tap through ${I.right}</span></div>`;
    } else if (o.cta) {
    ...
    return `<div class="canvas ${size} st ${feed ? "feed " : ""}${o.fmt === "x" ? "x " : ""}${bg}" data-still="${o.still}" style="${style}">${bg === "paper" ? '<div class="ruled"></div>' : bg === "sky" ? '<div class="ruled"></div>' : ""}${inner}</div>`;
  }
  ...
  function fit(root = document) {
    for (const el of root.querySelectorAll(".st")) {
      ...
      const over = () => [viz, ...viz.querySelectorAll("*")].some((n) => n.scrollWidth > n.clientWidth + 2 && getComputedStyle(n).overflow === "visible" && n.clientWidth > 0 && !n.matches("svg, svg *"));

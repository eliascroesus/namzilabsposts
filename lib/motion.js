...
(function () {
  ...
  function hydrateMarks(root = document) {
    ...
    for (const el of root.querySelectorAll(".mark[data-src]")) {
      ...
      if (!m) continue;
      // tiles (tools without an official mark) size their letters to the box
      const w = el.offsetWidth;
      el.innerHTML = m.svg
        ? m.svg
        : `<span class="tile" style="background:${m.color}${w ? `;font-size:${(w * 0.42).toFixed(1)}px` : ""}">${m.short}</span>`;
      if (!el.title) el.setAttribute("aria-label", m.name || el.dataset.src);
    }
  }

  /** The Namzilabs mark as an inline SVG: 01 Between, mono (two tools, the overlap filled in). */
  /* The mark: two rings, overlapping, with nothing in between (the overlap is left empty,
     so whatever sits behind the mark shows through). This is the one definition; banners,
     highlights, posts and videos all draw it through NZ.RINGS / NZ.MARK. */
  const RINGS = (c = "#14141c", w = 4.6) => `<circle cx="25" cy="32" r="15" fill="none" stroke="${c}" stroke-width="${w}"/><circle cx="39" cy="32" r="15" fill="none" stroke="${c}" stroke-width="${w}"/>`;
  const MARK = (c = "#14141c", w) => `<svg viewBox="0 0 64 64" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">${RINGS(c, w)}</svg>`;
  // the small app-icon version used in post and video corners: white rings on an ink tile
  const LOGO = `<svg viewBox="0 0 64 64" xmlns="http://www.w3.org/2000/svg" aria-hidden="true"><rect width="64" height="64" rx="16" fill="#14141c"/><g transform="translate(32 32) scale(0.8) translate(-32 -32)">${RINGS("#fff", 5)}</g></svg>`;
  ...
  window.NZ = Object.assign(window.NZ || {}, { ease, bezier, spring, Timeline, counter, fmt, splitWords, hydrateMarks, hydrateLogos, mount, still, LOGO, RINGS, MARK });
})();
